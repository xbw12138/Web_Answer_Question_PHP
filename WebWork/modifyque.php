...
require_once 'api/_getquestion.php'
?>
<style>
...
C.矿山、金属冶炼单位    D.危险物品使用单位</h6>';
        ...
        <input type="hidden" id="idd" value="<?php echo $_GET['id'];?>"/>
        <?php
            $array=getQuestion($_GET['type'],$_GET['id']);
            $problem="";
            $daan="";
        ...
        <form>
            <div>
                ...
                <div id="wrapper">
                    <?php
                    $str = $daan;
                    $A = preg_match('/.*A.*/', $str, $matches);
                    $B = preg_match('/.*B.*/', $str, $matches);
                    $C = preg_match('/.*C.*/', $str, $matches);
                    $D = preg_match('/.*D.*/', $str, $matches);
                    ?>
                    <span><label><input type="checkbox"class="input_check" value="A" id="check1" <?php  echo $A?"checked":"";?>><label>A</label></span><br>
                    <span><label><input type="checkbox"class="input_check" value="B" id="check2" <?php  echo $B?"checked":"";?>><label>B</label></span><br>
                    <?php if(isset($_GET['type'])&&$_GET['type']=="panduan"){

                    }else{
                        echo '<span><label><input type="checkbox"class="input_check" value="C" id="check3" ';
                        echo $C?"checked":"";
                        echo '><label>C</label></span><br>
                    <span><label><input type="checkbox"class="input_check" value="D" id="check4" ';
                        echo $D?"checked":"";
                        echo '><label>D</label></span><br>';
                    }?>
